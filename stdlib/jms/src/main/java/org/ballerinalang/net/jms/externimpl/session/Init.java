...
package org.ballerinalang.net.jms.externimpl.session;
...
public class Init {
    ...
    public static Object init(Strand strand, ObjectValue sessionObj, ObjectValue connectionObj,
                                       String ackMode) {
        Connection connection = (Connection) connectionObj.getNativeData(JmsConstants.JMS_CONNECTION);

        Session session;
        try {
            session = createSession(connection, ackMode);
        } catch (JMSException e) {
            return BallerinaAdapter.getError("Error while creating session.", e);
        }
        sessionObj.addNativeData(JmsConstants.JMS_SESSION, session);
        return null;
    }

    private static Session createSession(Connection connection, String ackMode) throws JMSException {
        ...
        switch (ackMode) {
            ...
            case JmsConstants.AUTO_ACKNOWLEDGE_MODE:
                ...
                break;
            default:
                throw new BallerinaException("Unknown acknowledgment mode: " + ackMode);
        }
