...
public class WebSocketUtil {
    ...
    public static BMap<String, BValue> getError(Context context, Throwable throwable) {
        if (throwable.getMessage() == null) {
            return BLangVMErrors.createError(context, "Unexpected error occurred");
        } else {
    ...
    public static String sanitizeBasePath(String basePath) {
        ...
        if (!basePath.startsWith(WebSocketConstants.DEFAULT_BASE_PATH)) {
            basePath = WebSocketConstants.DEFAULT_BASE_PATH.concat(basePath);
        }

        if ((basePath.endsWith(WebSocketConstants.DEFAULT_BASE_PATH) && basePath.length() != 1)) {
