...
package org.ballerinalang.net.websocket.clientendpoint;
...
import org.ballerinalang.net.websocket.WebSocketClientConnectorListener;
...
public class InitEndpoint extends BlockingNativeCallableUnit {
    ...
    public void execute(Context context) {
        ...
        Struct clientEndpointConfig = clientEndpoint.getStructField(WebSocketConstants.CLIENT_ENDPOINT_CONFIG);
    ...
    private void populateClientConnectorConfig(Struct clientEndpointConfig,
        ...
        Struct secureSocket = clientEndpointConfig.getStructField(WebSocketConstants.ENDPOINT_CONFIG_SECURE_SOCKET);
        if (secureSocket != null) {
            WebSocketUtil.populateSSLConfiguration(clientConnectorConfig, secureSocket);
        } else {
            WebSocketUtil.setDefaultTrustStore(clientConnectorConfig);
        }
    }
    ...
    static class WebSocketClientHandshakeListener implements ClientHandshakeListener {
        ...
        public void onSuccess(WebSocketConnection webSocketConnection, HttpCarbonResponse carbonResponse) {
            ...
            webSocketClientEndpoint.put(WebSocketConstants.CLIENT_RESPONSE_FIELD,
                                        WebSocketUtil.createResponseStruct(context, carbonResponse));
            BMap<String, BValue> webSocketConnector = BLangConnectorSPIUtil.createObject(
                    context, WebSocketConstants.PROTOCOL_PACKAGE_HTTP, WebSocketConstants.WEBSOCKET_CONNECTOR);
        ...
        public void onError(Throwable throwable, HttpCarbonResponse response) {
            if (response != null) {
                BMap<String, BValue> webSocketClientEndpoint = ((BMap<String, BValue>) context.getRefArgument(0));
                webSocketClientEndpoint.put(WebSocketConstants.CLIENT_RESPONSE_FIELD,
                                            WebSocketUtil.createResponseStruct(context, response));
            }
            countDownLatch.countDown();
