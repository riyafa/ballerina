...
/**
 * {@code Close} is the Close action implementation of the WebSocket Connector.
 */
@BallerinaFunction(
            ...
            if (timeoutInSecs < 0) {
                ...
                if (!countDownReached) {
                    String errMsg = String.format(
                            ...
                            timeoutInSecs);
                    context.setReturnValues(BLangVMErrors.createError(context, errMsg));
                }
            }
        } catch (InterruptedException err) {
            context.setReturnValues(
                    BLangVMErrors.createError(context, "Connection interrupted while closing the connection"));
